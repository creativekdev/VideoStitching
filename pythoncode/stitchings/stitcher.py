...
class Stitcher:
    ...
    def stitch(self, images, feature_masks=[]):
        ...
        imgs, masks, self.corners, sizes = self.warp_low_resolution(imgs, cameras)
        self.prepare_cropper(imgs, masks, self.corners, sizes)
        imgs, masks, self.corners, sizes = self.crop_low_resolution(
            imgs, masks, self.corners, sizes
        )
        self.estimate_exposure_errors(self.corners, imgs, masks)
        self.seam_masks = self.find_seam_masks(imgs, self.corners, masks)

        imgs = self.resize_final_resolution()
        imgs, masks, self.corners, sizes = self.warp_final_resolution(imgs, cameras)
        imgs, masks, self.corners, sizes = self.crop_final_resolution(
            imgs, masks, self.corners, sizes
        )
        self.set_masks(masks)
        imgs = self.compensate_exposure_errors(self.corners, imgs)
        self.seam_masks = self.resize_seam_masks(self.seam_masks)

        self.initialize_composition(self.corners, sizes)

        #mask_json = pickle.dump(self.seam_masks)
        #with open('mask.json' , 'w') as f:
        #    pickle.dump(self.seam_masks , f)    
        self.blend_images(imgs, self.seam_masks, self.corners)
        return self.create_final_panorama()
    
    def easy_stitch(self , images):
        self.images = Images.of(
            images, self.medium_megapix, self.low_megapix, self.final_megapix
        )
